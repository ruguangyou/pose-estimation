...
void Optimizer::fullBA() {
    int numKeyframes = _pMap->_pKeyframes.size();
    ...
    int numMapPoints = _pMap->_pMapPoints.size();
    double** delta_x = new double*[numMapPoints];
    ...
    int count = 0;
    for (int i = 0; i < numKeyframes-1; i++) {
        for (auto& mapPointID : _pMap->_pKeyframes[i]->mapPointIDs) {
            if (visitedMapPoint.find(mapPointID) != visitedMapPoint.end())
                continue;
            visitedMapPoint[mapPointID] = true;
            ...
            std::vector<int> frameIDs;
            for (auto& frameAndPixel : pMapPoint->pixels) {
                int idx = frameAndPixel.first;
                // frameAndPixel->first is frameID, >= n means the frame is within the sliding window.
                if (idx >= 0) {
                    frameIDs.push_back(frameAndPixel.first);
                    delta.push_back(delta_pose[idx]);
                }
            }
            ...
            // problem.AddResidualBlock(fullBACost, NULL, delta);
        }
    }

    double weight = 1e3;
    ceres::CostFunction* startCost = new ceres::AutoDiffCostFunction<OriginCostFunction, 6, 6>(new OriginCostFunction(_pMap->_pKeyframes[0], weight));
    problem.AddResidualBlock(startCost, NULL, delta_pose[0]);
    ceres::CostFunction* endCost = new ceres::AutoDiffCostFunction<OriginCostFunction, 6, 6>(new OriginCostFunction(_pMap->_pKeyframes.back(), weight));
    problem.AddResidualBlock(endCost, NULL, delta_pose[numKeyframes-1]);
