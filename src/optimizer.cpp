...
Optimizer::Optimizer(const cfsd::Ptr<Map>& pMap, const cfsd::Ptr<FeatureTracker>& pFeatureTracker, const cfsd::Ptr<ImuPreintegrator>& pImuPreintegrator, const cfsd::Ptr<CameraModel>& pCameraModel, const bool verbose)
    ...
    _marginalizeFrames = Config::get<int>("marginalizeFrames");
...
void Optimizer::motionOnlyBA(const cv::Mat& img) {
    ...
    size_t landmarkID;
    for (int i = (_pMap->_pKeyframes.size() > WINDOWSIZE + _marginalizeFrames) ? -_marginalizeFrames : 0; i < actualSize; i++) {
        // If number of keyframes is bigger than windowsize, it means there are frames needed marginalization.
    ...
    for (const auto& l : landmarks) {
        ...
        int errorTerms = l.second.size();
        if (errorTerms < 2) continue;
        delta_pose_img.clear();
        for (int i = 0; i < errorTerms; i++) {
        ...
        for (int i = 0; i < l.second.size(); i++) {
            ...
            error(2*i) = _invStdT(0,0) * (_fx * x / z + _cx - mp->pixel.x);
            error(2*i+1) = _invStdT(1,1) * (_fy * y / z + _cy - mp->pixel.y);

            // F.block<2,3>(2*i, 6*i) = _invStdT * d_e_pcam * _pCameraModel->_T_CB.so3().matrix() * Sophus::SO3d::hat(temp);
            F.block<2,3>(2*i, 6*i+3) = -_invStdT * d_e_pcam * _pCameraModel->_T_CB.so3().matrix();
            F.block<2,3>(2*i, 6*i) = -F.block<2,3>(2*i, 6*i+3) * Sophus::SO3d::hat(temp);

            // // E.block<2,3>(2*i, 0) = _invStdT * d_e_cam * _pCameraModel->_T_CB.so3().matrix() * windowFrame->R.inverse().matrix();
            // E.block<2,3>(2*i, 0) = -F.block<2,3>(2*i, 6*i+3) * windowFrame->R.inverse().matrix();
        }
    ...
    cv::Mat out;
    cv::vconcat(img, cv::Mat::zeros(20, img.cols, CV_8U), out);
    std::stringstream text;
    text << "#keyframes: " << _pMap->_pKeyframes.size()-1 << ", #landmarks: " << _pMap->_numLandmarks << ", #points in current frame:" << points.size();
    cv::putText(out, text.str(), cv::Point(4, out.rows-4), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255), 1);
